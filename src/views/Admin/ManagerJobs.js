...
import CustonPreviewEmployerView from '../../components/UI/CustonPreviewEmployerView';
/* Table Bootstrap 2 */
...
import { createJobOffer, copyJobsOffers, disableJobsOffers,/* getJobsOffersEmployers, */getJobsOffers } from '../../actions/jobsOffersActions';
import { getFilterAdmin, setFilterAdmin, getEmployerById } from '../../actions/usersActions';
...
class ManagerJobs extends Component {
    ...
    columnsGridJobsOffers = [
        {
            ...
            formatter: (cellContent, row) => {
                ...
                return(
                    <Row className="justify-content-center align-items-center">
                        ...
                        <ReactTooltip id={"btnViewEmployer_" + row.id}>
                        </ReactTooltip>  
                        ...
                        <ButtonToolbar>
                            <ButtonGroup size="sm">
                                <Button data-for={"btnEditJobOffer_" + row.id} data-tip={"Editar oferta cód: " +  row.codigo}  color="primary" onClick={(e) => { 
                                    
                                    this.props.history.push({
                                        pathname: `/area/admin/ofertas/editarOferta/${row.idGuid}`,
                                ...
                                <Button data-for={"btnViewPostulantJobOffer_" + row.id} data-tip={"Ver postulantes oferta cód: " +  row.codigo} color="success" onClick={(e) => { 
                                     this.props.history.push({
                                        pathname: `/area/admin/ofertas/postulaciones/${row.idGuid}`,
                                        //search: '?idJobOffer=' + row.idGuid,
                                        state: { idJobOffer: row.idGuid }
                                    });
                                } } >
                                ...
                                <Button data-for={"btnViewEmployer_" + row.id} data-tip={"Ver empleador oferta cód: " +  row.codigo} color="secondary" onClick={(e) => { 
                                   
                                    this.showCompany(row.idEmpleador);
                               ...
                               {/*  <Button data-for={"btnPrivateJobOffer_" + row.id} data-tip={"Dar de baja oferta cód: " +  row.codigo} color="danger" onClick={(e) => { } } >
            ...
            formatter: (cellContent, row) => {
                ...
                return(
                <div style={{paddingRight: "35px"}}>{row.nombreEmpleador}</div>
                );
            },
            style: { 
                'whiteSpace': 'nowrap',
    ...
    constructor(props) {
        ...
        //console.log(this.props.filterAdmin);
        ...
        this.state = {
            ...
            searchCode: '',
            searchText: '',
            selectedSearchEstado: (this.props.filterAdmin ? this.props.filterAdmin: null) || opcionesStateJobOffer[0] ,
    ...
    UNSAFE_componentWillReceiveProps(nextProps, nextState){
        ...
        const { resultJobOfferCreated, resultJobOfferCopied, resultJobOfferDisable, jobsOffersEmployersList, filterAdmin, resultGetEmployerById } = nextProps;
        ...
        if(filterAdmin !== this.props.filterAdmin)
        {
            /*this.setState({
                selectedSearchEstado: filterAdmin
            });*/
        ...
        if(this.props.jobsOffersEmployersList !== jobsOffersEmployersList){
            //console.log(jobsOffersEmployersList);
            if(jobsOffersEmployersList){

                if(isIterable(jobsOffersEmployersList.data.data)) {
                    ...
                    const dataGridJobsOffers = listJobsOffers.map( (item, index) => {
                        ...
                        const nombreEmpleador = item.empleador.bPersonaJuridica ? 
                        (item.empleador.nombreComercialEmpresa || item.empleador.razonSocial) : 
                        (item.empleador.nombrePersona + " " + item.empleador.apellidoPersona);
                        ...
                        return { 
                            ...
                            cantidadPostulantes: item.cantidadPostulantes, 
                            bAnulado: item.bAnulado, 
                            idEmpleador: item.frk_Empleador ? item.frk_Empleador.id : null,
                    ...
                    if(this.state.bSearchByFilter && this.state.pageNumber > 1){
                        ...
                        this.setState({
                            pageNumber: 1 ,
                            bSearchByFilter: false,
                            totalRegister: jobsOffersEmployersList.data.itemsCount,
        ...
        if(this.props.resultJobOfferCreated !== resultJobOfferCreated){
            ...
            if(resultJobOfferCreated.data){
                ...
                setTimeout(() => {
                    ...
                    this.props.history.push({
                        pathname: `/area/admin/ofertas/crearOferta/${ resultJobOfferCreated.data.id }`,
        ...
        if(this.props.resultJobOfferCopied !== resultJobOfferCopied){
            ...
            if(resultJobOfferCopied.data){
                ...
                setTimeout(() => {
                    ...
                    this.props.history.push({
                        pathname: `/area/admin/ofertas/crearOferta/${ resultJobOfferCopied.data.id }`,
        ...
        if(this.props.resultGetEmployerById !== resultGetEmployerById){

            //console.log(resultGetEmployerById);
            let content = <CustonPreviewEmployerView employer={{...resultGetEmployerById.employer}}></CustonPreviewEmployerView>
            this.setOpenModal(true, 'lg', 'Listo!', 'modal-info', "Empleador", false, content);
        }
    }

    componentDidMount(){
        
        this.props.getFilterAdmin();
        /*this.props.getJobsOffersEmployers({ 
            id: null, 
            pageSize: this.state.pageSize, 
            pageIndex: this.state.pageNumber - 1, 
            keySearch: "",
    ...
    handlePageChange(pageNumber, pageSize) {
        ...
        this.props.getJobsOffers({ id: null, pageSize: pageSize, pageIndex: pageNumber - 1,
            ...
            startDate: this.state.searchStartDate, 
            endDate: this.state.searchEndDate, 
            bShowDisableRegisters: this.state.bShowDisable, sortField: this.state.sortFieldGridJobsOffers, sortOrder: this.state.sortOrderGridJobsOffers });
    ...
    previewJobOffer = (idOffer) => {
        ...
        const { bPersonaJuridica, cbEmpleadorConficencial, descripcionEmpresaConfidencial, puesto, selectCategoriaEmpleo, descripcionPuesto, cbDiscapacidad, informatica, cbInformatica ,cbRemotoTodoMundo, obsRemote, 
            ...
            cbMostrarRenta, rentaMaxima, tipoMoneda, tipoPago, periodicidadPago, acercaPago, beneficios, cbHomeOffice, cbHorarioFlexible, allowPlacesToWorkRemotly, idiomsRequire, questionsList, placeToWork, experiencies, empleador} = this.dataFormJobOffer[idOffer] || {};
        ...
        const urlImageEmployer = `${webConfig.urlImages}/${empleador.rutaLogoEmpresa}`;
        
        let content = 
            <CustomJobOfferPreview dataForm={{
                bPersonaJuridica: bPersonaJuridica,
    ...
    showCompany = (idEmployer) => {
        this.props.getEmployerById({idEmployer:idEmployer});
    }
    ...
    onClickSearchCode = (code) => {
        ...
        this.props.getJobsOffers({ id: null, pageSize: this.state.pageSize, pageIndex: 0, keySearch: "", code: code, bShowDisableRegisters: this.state.bShowDisable });
    }
    ...
    onClickClearFilter = (e) => {
        ...
        this.setState({/*pageNumber: 1,*/searchCode: '', searchText: '', searchStartDate: null, searchEndDate: null, selectedSearchEstado: opcionesStateJobOffer[0], bShowDisable: false, code: '' });
    }
...
const mapStateToProps = state => ({
    filterAdmin: state.userAuth.filterAdmin,
    ...
    jobsOffersEmployersList: state.jobsOffers.jobsOffersList,//state.jobsOffers.jobsOffersEmployersList,
    resultGetEmployerById: state.userAuth.resultGetEmployerById
});
